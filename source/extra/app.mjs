...
export function mount(root,host={},bootstrap={}) {
 if(!session)session={screen:Object.keys(bootstrap).length?'focus':'scores',school:'yonsei',dept:'yonsei:경제학부',pct:{k:null,m:null,t1:null,t2:null,e:null,h:null,...bootstrap},std:{k:null,m:null,t1:null,t2:null},language:'unknown',grades:[null,null,null,null],confirmed:false,meme:true};
 const state=session;
 function render() {
  ...
  root.querySelectorAll('[data-input]').forEach(x=>x.addEventListener('input',()=>{
   ...
   if(path.length===2)state[path[0]][path[1]]=n;else state[path[0]]=n;
  }));
 ...
 function calculationHtml(d,r){
  const c=r.conversion??(['hanyang'].includes(d.school)?inquiryConversion(d.school,state.pct):null);
  return `<section class="rx-section"><h3>점수 계산, 과정까지</h3>${r.formula?`<div class="rx-formula">${escape(r.formula)}</div>`:''}<p class="jr-muted">K·M: 국수 표준점수 / T: 백분위로 자동 조회한 대학별 탐구 변환점수 / S: 탐구 표준점수. 성적표의 표준점수는 성적 입력에서 넣어주세요.</p>${c?`<div class="rx-details"><strong>탐구 자동 변환${c.year?' · '+c.year+'학년도 참고표':''}</strong><p>생윤 ${state.pct.t1??'—'} → ${c.values[0]??'확인 대기'} / 한지 ${state.pct.t2??'—'} → ${c.values[1]??'확인 대기'}</p><p class="jr-muted">${escape(c.note)}</p>${c.source?`<p><a href="${escape(c.source)}" target="_blank" rel="noopener noreferrer">변환표 출처</a> · ${escape(c.verification)}</p>`:''}</div>`:''}<div class="rx-result" aria-live="polite"><span>${c?.year?'2026 변환표 × 2027 반영식 참고점수':d.school==='skku'?'가중백분위 참고값':'입력값에 따른 수능 환산점수'}</span><strong>${r.value===null?'산출 보류':number(r.value,d.school==='sogang'?2:4)}</strong>${r.value===null?`<span>${escape(r.missing.join(' · '))}</span>`:`<code>${escape(r.substitution)}</code>`}</div><p class="jr-muted">${escape(r.extra)}</p><p class="jr-muted">다른 연도의 환산점수를 직접 빼서 합불을 판단하지 않습니다.</p>${sourceHtml(d.school)}</section>`;
 }
 ...
 function allDepartments(){
  return `<h3>서연고 · 서성한 전부 보기</h3><p class="jr-muted">${departments.length}개 주요 모집단위 · 학과를 누르면 적용 산식과 근거로 이동합니다.</p>${schools.map(s=>`<section class="rx-section"><h4>${s.name}</h4><div class="rx-dept-list">${departments.filter(d=>d.school===s.id).map(d=>{const r=calculation(d,state),e=r.eligibility;return `<button type="button" class="rx-dept" data-dept="${escape(d.id)}"><span><strong>${escape(d.name)}</strong><small>${escape(d.pool)}</small></span><span>${tag(e.status==='blocked'?'불가':e.status==='hold'?'확인':'조건충족',e.status==='blocked'?'critical':'neutral')}<small>${r.value===null?'환산 보류':number(r.value,2)+(r.conversion?.year?' (2026표 참고)':'')} · ${d.gun}군</small></span></button>`}).join('')}</div></section>`).join('')}`;
 }
 function evidence(){return `<h3>밈은 가볍게, 근거는 확실히</h3><label class="rx-check"><input type="checkbox" data-action="meme" ${state.meme?'checked':''}> 밈 말투 켜기</label><div class="rx-facts"><div><span>검토일</span><strong>2026.09.23</strong></div><div><span>적용 과목</span><strong>화작 · 미적 · 생윤 · 한지</strong></div><div><span>대상 전형</span><strong>기초생활수급자 지원 특별전형</strong></div><div><span>합격 확률</span><strong>산출하지 않음</strong></div></div><p>‘할만한데?’는 과거 공개 학생과의 과목별 비교 표현입니다. ‘씹가능’을 합격 보장으로 표시하지 않습니다.</p><p>서울·연세·고려·서강은 확인된 식과 입력값으로 계산합니다. 탐구 변환점수는 백분위로 자동 조회합니다. 연세대는 공식 2026 표, 고려·한양대는 기존 레이더의 2026 언론 전재 표이며 원문 재확인 미완료 표시가 붙습니다. 서강대는 확인된 표 확보 전까지 보류합니다. 서울대는 성적표의 탐구 표준점수를 씁니다.</p><p>성균관대 나군은 영어 변환백분위표 확인 전까지 계산을 보류합니다. 성균관대 가·다군과 한양대는 아직 없는 2027 세부 환산표·정규화 상수·학생부 점수를 만들지 않고 반영 구조와 보류 이유를 보여줍니다.</p><p>같은 군에는 한 곳만 지원할 수 있습니다. 연세·고려는 가군으로 겹치며, 서울·서강은 나군으로 겹칩니다. 최종 모집인원은 수시 이월 후 다시 확인해야 합니다.</p><div class="rx-source-list">${schools.map(s=>`<div>${sourceHtml(s.id)}<p class="jr-muted">${sources[s.id].pages}</p></div>`).join('')}${sourceHtml('yonseiResult')}${sourceHtml('sogangResult')}</div>`;}
 render();
}
